...
void LT::PrintParseTree(int indent, int level)
{
...
Val LT::Eval(HashMap<Val>& state, Val& val)
{
    return Eval(state);
}
Val LT::Eval(HashMap<Val>& state)
{
    Val a = e1->Eval(state);
    Val b = e2->Eval(state);
    if (a->isErr())
    ...
    return ErrVal(std::string("< operator cannot be applied to [") + a->toString() + "," +  b->toString() + "]");
}
LT::LT(E* e1, E* e2)
